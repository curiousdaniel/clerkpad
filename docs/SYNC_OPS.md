...
The same client connection also subscribes to **`clerkbid:announce`** for super-admin global in-app toasts (`POST /api/admin/ably-announce/`). Users receive those whenever they are signed in with a valid `vendorId`, even if no event is selected.
